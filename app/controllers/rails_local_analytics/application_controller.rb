module RailsLocalAnalytics
  class ApplicationController < ActionController::Base
    ...
    def current_tracked_requests_by_day
     TrackedRequestsByDayPage.where(url_hostname: params[:site]).between_dates(from_date, to_date)
    end

    def previous_tracked_requests_by_day
     TrackedRequestsByDayPage.where(url_hostname: params[:site]).between_dates(previous_from_date, previous_to_date)
    end
