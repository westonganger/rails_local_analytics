...
class CreateAnalyticsTables < ActiveRecord::Migration[6.0]
  def up
    ...
    add_index :tracked_requests_by_day_page, [:url_hostname, :url_path, :date], name: "index_tracked_requests_by_day_page_on_url_and_date"
...
# Tracking additional models

A key aspect of this library is its simplicity and small API. For major functionality customizations we encourage you to first remove this gem from your Gemfile and then copy this gems code directly into your app.

Having said that it can be done using the following technique:

```ruby
# config/initializers/rails_local_analytics.rb

Rails.configuration.to_prepare do
  RailsLocalAnalytics::RecordRequestJob.class_eval do
    def models
      MODELS + [YourCustomModel]
    end
  end
end
```
...
Imitated some parts of [`active_analytics`](https://github.com/BaseSecrete/active_analytics). Thanks to them for the good ideas.
